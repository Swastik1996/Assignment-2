...
$\Longrightarrow$ Pr[$(\widetilde{p}$ $\textless$ $p(1 - \epsilon))$ $\bigcup$ $\widetilde{p}$ $\textgreater$ $p(1 + \epsilon)$] $\textless$ $\delta$ \\\\
$\Longrightarrow$ Pr[$\widetilde{p}$ $\textless$ $p(1 - \epsilon)$]
+ Pr[$\widetilde{p}$ $\textgreater$ $p(1 + \epsilon)$] $\textless$ $\delta$ \hfill{$\textit{(Both events are disjoint)}$}\\\\
